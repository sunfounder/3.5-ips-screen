...
このセクションでは、Raspberry Pi に適したオペレーティングシステムのインストール方法、Wi-Fi の設定、そしてコードを簡単に実行できるようリモートアクセスを有効にする手順を説明します。

すでに Raspberry Pi の操作に慣れていて、コマンドラインにアクセスできる場合は、このセクションをスキップできます。
